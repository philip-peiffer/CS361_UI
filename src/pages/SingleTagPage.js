...
function SingleTagPage({selections, setSelections}){
    ...
    const loadData = async () => {
        // fetch a single tag from the database
        const host = 'http://localhost:5000/'
        const {residency, species, region, district, tag} = selections
        let fetchTags = await fetch(host + `residency/${residency}/species/${species}/tags/${tag}/stats`)
